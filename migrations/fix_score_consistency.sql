-- ==========================================================
-- FIX: Student Progress Score Calculation Consistency
-- ==========================================================
-- This migration fixes the score discrepancy between actual test scores 
-- and scores shown on the dashboard.
--
-- ISSUE: student_progress was storing GREATEST score instead of LATEST score,
-- causing dashboards to show inflated scores that didn't match recent attempts.
--
-- FIX: Update submit_and_grade_test function to store LATEST score
-- ==========================================================
...
BEGIN
  ...
  -- FIXED: Store LATEST score, not GREATEST, to ensure dashboards show most recent performance
...
-- Also update existing student_progress records to use latest test submission scores
-- This ensures historical data is consistent
WITH latest_submissions AS (
  SELECT DISTINCT ON (user_id, course_id, level)
    user_id,
    course_id,
    level,
    raw_score_percentage,
    (raw_score_percentage >= 40) as passed
  FROM test_submissions
  ORDER BY user_id, course_id, level, test_date DESC
)
UPDATE student_progress sp
SET score = ls.raw_score_percentage,
    passed = sp.passed OR ls.passed,
    created_at = now()
FROM latest_submissions ls
WHERE sp.user_id = ls.user_id
  AND sp.course_id = ls.course_id
  AND sp.level = ls.level;
