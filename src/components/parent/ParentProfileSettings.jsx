...
const { FiUser, FiMail, FiPhone, FiSave, FiLoader, FiCheckCircle, FiAlertCircle, FiLock, FiKey } = FiIcons;

// Same account-management pattern as StudentSettings.jsx (updateProfile/updateEmail from
// AuthContext, authService.updatePassword) - all of those operate purely on the currently
// authenticated user's own profile row/auth record regardless of role, so this works identically
// for a parent account. updateProfile/updateEmail already update AuthContext's user state
// directly, so the Parent Portal sidebar/header (which reads user.name/user.email) picks up the
// change immediately with no extra wiring here.
const ParentProfileSettings = () => {
  const { user, updateProfile, updateEmail } = useAuth();
  const [activeTab, setActiveTab] = useState('profile'); // 'profile' or 'security'

  const [formData, setFormData] = useState({ name: '', email: '', mobile: '' });
  const [securityData, setSecurityData] = useState({ newEmail: '', newPassword: '', confirmPassword: '' });
  ...
  useEffect(() => {
    if (user) {
      setFormData(prev => {
        const newData = { name: user.name || '', email: user.email || '', mobile: user.mobile || '' };
  ...
  const handleProfileSubmit = async (e) => {
    ...
    try {
      const result = await updateProfile({ name: formData.name, mobile: formData.mobile });
  ...
  return (
    <div className="p-0">
      <div className="max-w-4xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 dark:text-white">Profile Settings</h1>
        ...
        >
          <div className="bg-gradient-to-r from-amber-600 to-orange-600 p-4 sm:p-8 flex flex-col sm:flex-row items-center gap-4 sm:gap-6 text-center sm:text-left">
            <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-white/15 flex items-center justify-center text-white text-2xl sm:text-3xl font-bold shadow-xl border-4 border-white/10 flex-shrink-0">
              {formData.name.charAt(0).toUpperCase() || 'P'}
            </div>
            <div>
              <h2 className="text-xl sm:text-2xl font-bold text-white mb-1">{formData.name || 'Parent'}</h2>
              <span className="px-3 py-1 bg-white/10 rounded-full text-xs font-medium text-amber-50 border border-white/20 inline-block">
                Parent Account
              </span>
            </div>
          </div>
          ...
          <div className="p-4 sm:p-8">
            ...
            {/* TAB: PROFILE */}
            {activeTab === 'profile' && (
              <form onSubmit={handleProfileSubmit} className="max-w-xl">
                <div className="space-y-6">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full p-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-amber-500 outline-none transition-all dark:text-white"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        onChange={handleChange}
                        placeholder="+91..."
                        className="w-full p-3 pl-10 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-amber-500 outline-none transition-all dark:text-white"
                      />
                ...
                <div className="mt-10 pt-6 border-t border-gray-100 dark:border-gray-700 flex justify-end">
                  <button
                    type="submit"
                    disabled={loading}
                    className="px-8 py-3 bg-amber-600 text-white rounded-xl font-bold hover:bg-amber-700 transition-all flex items-center gap-2 shadow-lg shadow-amber-200 dark:shadow-none disabled:opacity-70 disabled:cursor-not-allowed"
                  >
              ...
              <div className="space-y-10 max-w-2xl">
                <form onSubmit={handleEmailUpdate} className="space-y-6">
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2 border-b border-gray-100 dark:border-gray-700 pb-2">
                    <SafeIcon icon={FiMail} className="text-amber-600" /> Change Email Address
                  </h3>
                  ...
                  <div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                      <input
                        ...
                        className="flex-1 p-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-amber-500 outline-none transition-all dark:text-white"
                      />
                      <button
                        type="submit"
                ...
                <form onSubmit={handlePasswordUpdate} className="space-y-6">
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2 border-b border-gray-100 dark:border-gray-700 pb-2">
                    <SafeIcon icon={FiKey} className="text-amber-600" /> Change Password
                  </h3>
                  ...
                  <div className="flex justify-center sm:justify-end">
                    <button
                      ...
                      className="w-full sm:w-auto px-8 py-3 bg-amber-600 text-white rounded-xl font-bold hover:bg-amber-700 transition-all disabled:opacity-50"
                    >
