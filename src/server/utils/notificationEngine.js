...
export function buildTestCompletionEmail({ studentName, testName, courseName, score, totalQuestions, correctAnswers, scaledScore, testDate, appUrl, reportUrl, modularScores }) {
    ...
    const isACTEmail = nameToVerify && String(nameToVerify).toUpperCase().includes('ACT');
    ...
    if (isACTEmail) {
        ...
        return `<!DOCTYPE html><html><head><meta charset="utf-8">${BASE_STYLES}</head><body><div class="wrapper"><div class="card"><div class="header"><h1>📝 ACT Test Completed!</h1><p>${appName} — ${formattedDate}</p></div><div class="body"><p class="intro-heading">Hello ${studentName || 'Student'},</p><p class="intro-text">Your test results are ready. Here is your performance summary:</p><div style="background: rgba(255,255,255,0.03); border-radius: 12px; padding: 20px; border: 1px solid rgba(255,255,255,0.08); margin-bottom: 24px;"><p style="margin-bottom: 8px; font-size: 15px;"><strong style="color: #94a3b8;">Student:</strong> <span style="color: #ffffff; font-weight: 600;">${studentName}</span></p><p style="margin-bottom: 8px; font-size: 15px;"><strong style="color: #94a3b8;">Test Name:</strong> <span style="color: #ffffff; font-weight: 600;">${courseName || testName}</span></p><p style="margin-bottom: 8px; font-size: 15px;"><strong style="color: #94a3b8;">Test Date:</strong> <span style="color: #ffffff; font-weight: 600;">${formattedDate}</span></p><p style="margin-bottom: 0; font-size: 15px;"><strong style="color: #94a3b8;">Status:</strong> <span style="color: #4ade80; font-weight: 600;">Completed</span></p></div><p class="section-title">📊 Test Summary</p><div style="background: rgba(255,255,255,0.03); border-radius: 12px; padding: 20px; border: 1px solid rgba(255,255,255,0.08); margin-bottom: 24px;"><p style="margin-bottom: 12px; font-size: 15px;"><strong style="color: #94a3b8;">Student Name:</strong> <span style="color: #ffffff; font-weight: 600;">${studentName}</span></p><p style="margin-bottom: 12px; font-size: 15px;"><strong style="color: #94a3b8;">Test Name:</strong> <span style="color: #ffffff; font-weight: 600;">${courseName || testName}</span></p><p style="margin-bottom: 12px; font-size: 15px;"><strong style="color: #94a3b8;">Status:</strong> <span style="color: #4ade80; font-weight: 600;">Completed</span></p><p style="margin-bottom: 12px; font-size: 15px;"><strong style="color: #94a3b8;">Total Questions:</strong> <span style="color: #ffffff; font-weight: 600;">${totalQuestions}</span></p><p style="margin-bottom: 12px; font-size: 15px;"><strong style="color: #94a3b8;">Correct Answers:</strong> <span style="color: #4ade80; font-weight: 600;">${correctAnswers}</span></p><p style="margin-bottom: 12px; font-size: 15px;"><strong style="color: #94a3b8;">Incorrect Answers:</strong> <span style="color: #f87171; font-weight: 600;">${incorrectAnswers}</span></p><p style="margin-bottom: 0; font-size: 15px;"><strong style="color: #94a3b8;">Accuracy:</strong> <span style="color: #818cf8; font-weight: 600;">${accuracyPct}%</span></p></div>${modularHtml}<div class="tip-box">💡 Review your incorrect answers to improve your next score. Every mistake is a learning opportunity!</div><a class="cta" href="${finalReportUrl}">View Full Report →</a></div><div class="footer">${appName} • Results are calculated based on all modules completed.</div></div></div></body></html>`;
    }
...
// grandTotals: {assigned, completed, missed} summed across every group in this one email - a
// student in more than one group whose deadline expired in the same run gets ONE combined email
// (see notifications.js run-group-deadline-check), never one per group.
export function buildGroupDeadlineMissedContentEmail({ recipientName, studentName, isParent, groups = [], grandTotals = { assigned: 0, completed: 0, missed: 0 }, appUrl, reportUrl }) {
    const appName = process.env.APP_NAME || 'AIPrep365';
    const formatDate = (d) => d ? new Date(d).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' }) : 'N/A';
    const missedLabel = (status) => status === 'not_started' ? 'Not Started' : 'Partially Completed';

    const renderMissedItems = (items) => {
        if (!items || items.length === 0) return '';
        return `<div style="margin-top:6px;">
            <div style="font-size:11px; font-weight:800; color:#f87171; text-transform:uppercase; letter-spacing:0.05em; margin-bottom:4px;">Missed Topics:</div>
            ${items.map(it => `<div style="padding:4px 0; border-bottom:1px solid rgba(255,255,255,0.05); font-size:13px; color:#e2e8f0;">
                <span style="display:inline-block; width:7px; height:7px; border-radius:50%; background:#f87171; margin-right:8px;"></span>${it.name}
                <span style="color:#94a3b8; font-size:11px;"> (${missedLabel(it.status)})</span>
            </div>`).join('')}
        </div>`;
    };
    ...
    const renderSubjectBlockForStudent = (s) => `
        <div style="margin-bottom:16px;">
            <div style="font-size:14px; font-weight:800; color:#f8fafc; margin-bottom:6px;">${s.subject}</div>
            <div style="font-size:13px; color:#cbd5e1; line-height:1.7;">
                Assigned: ${s.assigned} topic${s.assigned === 1 ? '' : 's'}<br/>
    ...
    // Parent email: subject blocks show only the counts; the actual topic names are listed once,
    // consolidated by subject, in a separate section afterward (renderParentMissedSection).
    const renderSubjectBlockForParent = (s) => `
        <div style="margin-bottom:16px;">
            <div style="font-size:14px; font-weight:800; color:#f8fafc; margin-bottom:6px;">${s.subject}</div>
            <div style="font-size:13px; color:#cbd5e1; line-height:1.7;">
                Assigned: ${s.assigned} topic${s.assigned === 1 ? '' : 's'}<br/>
    ...
    const renderParentMissedSection = () => {
        ...
        groups.forEach(g => {
            g.subjects.forEach(s => {
                if (!s.missedItems.length) return;
                if (!bySubject.has(s.subject)) bySubject.set(s.subject, []);
                bySubject.get(s.subject).push(...s.missedItems);
            });
        });
        if (bySubject.size === 0) return '';
        return `
            <p class="section-title">${studentName || 'The student'} missed the following content:</p>
            ${Array.from(bySubject.entries()).map(([subject, items]) => `
                <div style="margin-bottom:14px;">
                    ...
                    ${items.map(it => `<div style="padding:6px 0; border-bottom:1px solid rgba(255,255,255,0.05); font-size:14px; color:#e2e8f0;">
                        <span style="display:inline-block; width:8px; height:8px; border-radius:50%; background:#f87171; margin-right:10px;"></span>${it.name}
                        <span style="color:#94a3b8; font-size:12px;"> (${missedLabel(it.status)})</span>
                    </div>`).join('')}
                </div>
            `).join('')}
        `;
    ...
    const groupsHtml = groups.map(g => `
        <div style="margin-bottom:22px; padding-bottom:18px; border-bottom:1px solid rgba(255,255,255,0.06);">
            <p class="section-title" style="margin-bottom:12px;">📚 ${g.groupName || 'Student Group'} <span style="color:#64748b; font-weight:500; font-size:12px;">(ended ${formatDate(g.endDate)})</span></p>
            ${g.subjects.map(isParent ? renderSubjectBlockForParent : renderSubjectBlockForStudent).join('')}
        </div>
    `).join('');
