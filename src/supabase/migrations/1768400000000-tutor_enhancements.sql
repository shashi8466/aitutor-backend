-- ==========================================================
-- TUTOR DASHBOARD ENHANCEMENTS: GROUPS & ADVANCED GRADING
-- ==========================================================

-- 1. STUDENT GROUPS TABLE
...
-- 3. TEST RESPONSES TABLE (For question-wise analysis)
CREATE TABLE IF NOT EXISTS test_responses (
  id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  submission_id bigint REFERENCES test_submissions(id) ON DELETE CASCADE NOT NULL,
...
-- CREATE INDEXES
CREATE INDEX IF NOT EXISTS idx_test_responses_submission ON test_responses(submission_id);
...
-- 5. POLICIES FOR STUDENT GROUPS

-- Tutors can manage their own groups
CREATE POLICY "Tutors can manage own groups"
...
CREATE POLICY "Students can see their groups"
ON student_groups FOR SELECT TO authenticated
USING (
  EXISTS (SELECT 1 FROM group_members WHERE group_id = student_groups.id AND student_id = auth.uid())
);

-- 6. POLICIES FOR GROUP MEMBERS
CREATE POLICY "Tutors can manage group members"
ON group_members FOR ALL TO authenticated
USING (
  EXISTS (SELECT 1 FROM student_groups WHERE id = group_members.group_id AND (created_by = auth.uid() OR EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin')))
);

-- 7. POLICIES FOR TEST RESPONSES
CREATE POLICY "Users can view own responses"
ON test_responses FOR SELECT TO authenticated
USING (
  EXISTS (SELECT 1 FROM test_submissions WHERE id = test_responses.submission_id AND user_id = auth.uid()) OR
  EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin') OR
  EXISTS (
    SELECT 1 FROM test_submissions ts
    JOIN profiles p ON p.id = auth.uid()
    WHERE ts.id = test_responses.submission_id
    AND p.role = 'tutor'
    AND ts.course_id = ANY(p.assigned_courses)
  )
);

-- 8. UPDATE submit_and_grade_test FUNCTION TO STORE RESPONSES
-- Note: We drop and recreate it because we are adding response storage
