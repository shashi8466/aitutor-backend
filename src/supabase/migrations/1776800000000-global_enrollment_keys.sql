-- 1. Make course_id nullable in enrollment_keys table to allow global keys (which do not target a single course)
ALTER TABLE enrollment_keys ALTER COLUMN course_id DROP NOT NULL;

-- 2. Add columns to enrollment_keys to track key type and whether new courses should automatically enroll this key's users
...
-- 3. Update the database trigger function check_enrollment_key_course_consistency()
-- This ensures that for course-specific keys (course_id IS NOT NULL), we validate course matching.
-- For global keys (course_id IS NULL), we bypass the check, allowing users to enroll in any course.
...
BEGIN
    ...
    IF NEW.enrollment_key_id IS NOT NULL THEN
        ...
        -- Check for mismatch: only if key has a course_id (i.e. single-course key)
...
-- 4. Re-create the trigger to apply the updated check_enrollment_key_course_consistency function
...
-- 5. Update the validate_enrollment_key function to handle global keys (where course_id is NULL)
-- Returns 'All Courses' as course_name for global keys.
...
DECLARE
  ...
  v_course_name text;
BEGIN
  -- Get the key (case-insensitive lookups)
...
-- 6. Update the use_enrollment_key function
-- If a key is global (course_id IS NULL), it will automatically enroll the student in all active courses in the database.
-- If a key is course-specific (course_id IS NOT NULL), it maintains strict single-course enrollment checks.
...
BEGIN
  ...
  -- 🎯 GLOBAL ENROLLMENT KEY HANDLING
  IF v_validation.course_id IS NULL THEN
    -- Enroll the user in all courses they are not already enrolled in
  ...
  -- 🎯 CRITICAL: Strict Binding Check (For Single Course Keys)
...
-- 7. Add a trigger to automatically enroll existing global key users in new courses
-- If a course is created and a global key has auto_enroll_new_courses enabled, all students who enrolled using that key will be automatically enrolled.
CREATE OR REPLACE FUNCTION enroll_global_key_users_in_new_course()
RETURNS TRIGGER AS $$
DECLARE
    v_key RECORD;
BEGIN
    -- Loop through all global keys that have auto_enroll_new_courses enabled
    FOR v_key IN 
        SELECT id FROM enrollment_keys 
        WHERE key_type = 'global' AND auto_enroll_new_courses = true AND is_active = true
    LOOP
        -- Find all users who enrolled using this global key
        -- and enroll them in the newly created course if not already enrolled.
        INSERT INTO enrollments (user_id, course_id, enrollment_key_id, enrollment_method)
...
-- 8. Add a trigger to automatically delete enrollments when an enrollment key is deleted
-- This ensures database-level integrity, making sure that if a key is deleted (from the dashboard, RPC, or API),
-- any enrollments created with that key are revoked.
CREATE OR REPLACE FUNCTION delete_enrollments_on_key_deletion()
RETURNS TRIGGER AS $$
BEGIN
    DELETE FROM enrollments 
    WHERE enrollment_key_id = OLD.id;
